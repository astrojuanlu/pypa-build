...
def wheel_metadata(
    ...
    *,
    ...
    ``build_wheel``, and parses the resulting ``METADATA`` the same way the
    ``python -m build --metadata`` CLI does.

    :param source_dir: Project source directory
    :param isolated: Whether to invoke the backend in the current environment or
    ...
    :param check_dependencies: When not isolated, verify that the project's
                               ...
                               Ignored when ``isolated`` is true, where the
                               dependencies are always installed. Raises
                               :class:`build.DependencyError` if any are unmet.
    """
    if isolated:
    ...
    builder = ProjectBuilder(source_dir, runner=runner)
